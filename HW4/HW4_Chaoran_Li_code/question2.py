...
"""
Question 2:
Based on question 1, implement the following five algorithms to train a linear regression model for the Boston housing
data set. Report the root mean squared errors (RMSE) on the training and testing sets.
...
class MyLinearRegression:
    ...
    def fit(self, _X_trn, _y_trn):
        ...
        for t in range(self.steps):
            ...
            # if Nesterov
            if self.a_id == 3:
                h = np.dot(np.add(theta, np.dot(self.eta, v)), x.transpose())
